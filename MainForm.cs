...
namespace pcapTest
{
    public partial class MainForm : Form
    {
        ...
        private void listView1_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            ...
            Packet pack;
            pack = capturedPackets_list[key-1];
                    ...
                    if(getPacket)
                    {
                        ...
                        if (tcpPacket != null)
                        {
                            ...
                            if (l > 0)
                            {
                                ...
                                {
                                    hex1 += hex;
                                    raw = new byte[hex1.Length / 2];
                                    for (int i = 0; i < raw.Length; i++)
                                    {
                                        raw[i] = Convert.ToByte(hex.Substring(i * 2, 2), 16);
                                    }

                                    data = Encoding.ASCII.GetString(raw);
                                }
                                hex1 += hex;
                            }
                        }
                        ...
                        //var parspack = packet.ParsePacket(e.LinkLayerType, e.Data);
                        if (tcpPacket != null)
                        {
                            int srcPort = tcpPacket.SourcePort;
                            int dstPort = tcpPacket.DestinationPort;
                            var checksum = tcpPacket.Checksum;
                            var result = System.Text.Encoding.UTF8.GetString(tcpPacket.PayloadData);
                            String paydata = Encoding.ASCII.GetString(tcpPacket.PayloadData);
                            textBox2.Text = "";
                            textBox2.Text = "Packet number: " + key +
                                            ...
                                            //"\r\nPayload: " + (System.Text.Encoding.UTF8.GetString(tcpPacket.PayloadData).TrimEnd('\0'));
                                            "\r\nPayload: " + (data);
                ...
                case "ARP":
                    if (getPacket)
                    {
                        var arpPacket = (ARPPacket)pack.Extract(typeof(ARPPacket));
                        if (arpPacket != null)
                        {
                ...
                case "ICMPV6":
                    if (getPacket)
                    {
                        var icmpPacket = (ICMPv6Packet)pack.Extract(typeof(ICMPv6Packet));
                        if (icmpPacket   != null)
                        {                
                            textBox2.Text = "";
                            textBox2.Text = "Packet number: " + key +
                                            " Type: ICMP v6" +
                                            "\r\nType Code: 0x" + icmpPacket.Code.ToString("x") +
                                            "\r\nChecksum: " + icmpPacket.Checksum.ToString("x"); 
                                       //    "\r\nSource hw address" + icmpPacket.ParentPac.ToString("x") +
                ...
                case "IGMP":
                    if (getPacket)
                    {
                        var igmpPacket = (IGMPv2Packet)pack.Extract(typeof(IGMPv2Packet));
                        if (igmpPacket != null)
                        {
                            textBox2.Text = "";
                            textBox2.Text = "Packet number: " + key +
        ...
        public void Device_OnPacketArrival(object sender, CaptureEventArgs e)
        {
            ...
            //var paydata = packet.PayloadPacket.PayloadPacket.PayloadData;
         //   packet.PayloadData = paydata;
            //string result = System.Text.Encoding.Default.GetString(paydata);
            // add to the list
            ...
            if (ipPacket != null)
                {
                    ...
                    protocol_type = "ARP";
                    //protocol_type = arpPacket.ProtocolAddressType.ToString();
                    sourceIP = srcIp.ToString();
                    destinationIP = dstIp.ToString();



                    var protocolPacket = arpPacket.PayloadPacket;
